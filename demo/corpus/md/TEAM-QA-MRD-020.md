# TEAM-QA-MRD-020 - Organisation de l'equipe Assurance Qualite

| Champ | Valeur | Champ | Valeur |
|---|---|---|---|
| Reference | TEAM-QA-MRD-020 | Version | V4 |
| Type | Fiche d'organisation | Mise en activite | 2025-04-02 |
| Entite | Meridian Labs - Assurance Qualite | Redacteur | Sarah Delaunay |
| Statut | En activite | Approbateur | Marc Villeneuve |

## 1. Objet

Ce document decrit la composition et la repartition des activites de l'equipe Assurance Qualite de Meridian Labs.

## 2. Composition de l'equipe

L'equipe compte 4 collaborateurs au 2 avril 2025 :

| Role | Titulaire |
|---|---|
| Responsable Assurance Qualite | Sarah Delaunay |
| Auditrice interne et formation | Nadia Bouchard |
| Qualiticien processus | Farid Amrani |
| Alternante Assurance Qualite | Lea Girard |

## 3. Repartition des activites

Sarah Delaunay pilote la gestion documentaire, la revue de direction et la maitrise du changement. Nadia Bouchard pilote le programme d'audits internes et le plan de formation. Farid Amrani assure le traitement courant des non-conformites et le suivi des indicateurs de processus. Lea Girard appuie l'equipe sur la maitrise des enregistrements et la preparation des revues.

## 4. Documents associes

- PR-QA-MRD-001 - Gestion documentaire du systeme qualite

- PR-QA-MRD-009 - Gestion des non-conformites et des actions correctives (CAPA)

- PR-QA-MRD-010 - Programme d'audits internes

- PR-QA-MRD-011 - Revue de direction

## Historique des revisions

| Version | Date | Objet de la revision |
|---|---|---|
| V4 | 2025-04-02 | Arrivee de Lea Girard en alternance, effectif porte a 4 |
| V3 | 2024-01-22 | Creation du poste de qualiticien processus |

_Document fictif genere pour une demonstration technique. Meridian Labs est une organisation imaginaire._
