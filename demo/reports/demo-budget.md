# RAG evaluation - demo-budget

- run at: 2026-08-14T09:12:40+00:00
...
- judge: Missing satisfaction score detail; otherwise accurate.
...
5. Satisfaction déclarée des demandeurs : à évaluer mensuellement.

Ces indicateurs sont revus
...
- expected sources: PR-EXM-MRD-003
- not retrieved: -
- answer: La durée de rétention des sauvegardes n'est pas spécifiquement mentionnée dans le contexte fourni. Je ne sais pas basé sur les données indexées.
...
- judge: Missing mention of revocation automatic at expiration.
- expected sources: PR-ITS-MRD-004
- not retrieved: -
...
- judge: Missing details on non-grave cases and specific procedures.
- expected sources: PR-MI-MRD-001, PR-PV-MRD-002
- not retrieved: -
- answer: Une demande d'information médicale décrivant un effet indésirable suit un circuit spécifique de traitement. Selon la procédure PR-MI-MRD-001, les demandes signalant des effets indésirables sont transmises au service de pharmacovigilance conformément à la procédure PR-PV-MRD-002. Pour ce qui est des délais, un accusé de réception est envoyé au notificateur dans un délai de 2 jours ouvrés, et les dé
...
- judge: Incorrect document reference; misses quarterly restoration test.
...
- judge: Incorrect treatment deadlines; only minor non-conformity deadline is mentioned.
- expected sources: PR-QA-MRD-001, PR-QA-MRD-009
- not retrieved: PR-QA-MRD-001
- answer: Un taux d'accusé de réception de 85 pour cent à la mise en activité peut être considéré comme un problème, car il est en dessous du seuil de conformité de 90 pour cent. Ce taux doit être traité dans un délai de 45 jours si la non-conformité est classée comme majeure, ou dans un délai de 90 jours si elle est classée comme mineure, selon la procédure de gestion des non-conformités [Source: PR-QA-MRD
...
- answer: Les anomalies détectées lors de la revue des pistes d'audit doivent être traitées selon la procédure PR-QA-MRD-009, qui définit le traitement des non-conformités internes et externes. Ces non-conformités sont classées en majeures ou mineures, avec des délais de clôture respectifs de 45 jours pour les majeures et 90 jours pour les mineures. Une vérification de l'efficacité des actions correctives e
...
- judge: Fails to provide the required information about the duration.
...
- judge: RTO stated as 4 hours contradicts the reference answer of 8 hours.
- expected sources: PR-EXM-MRD-014
- not retrieved: -
- answer: L'objectif de reprise (RTO) retenu pour la messagerie et les outils collaboratifs est fixé à 4 heures, avec un RPO de 1 heure, vérifiés annuellement [Source: DR_RISKA-ITS-MRD-005.pdf].
